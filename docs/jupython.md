# Container `jupython.sif`

Specific information about the `jupython.sif` file goes here.
Modify as needed.

You may use the ``jupython.sif`` container file to familiarize yourself with [Singularity](<https://sylabs.io/docs/>),
...
* Download ``jupython.sif`` from [here](https://github.com/blzserdos/jupython/tree/main/containers)
* Import these files to your secure HPC environment
* Run ``singularity exec --no-home jupython.sif python3 --help``, to validate that you can run singularity. This command is expected to produce the standard python help message, starting like this:
...
1. The command
   ...
   ```
   singularity exec --home $PWD:/home jupython.sif python -c "import os; print(f'Hello World from {os.getcwd()}')"
   ```
...
2. Now, let's assume that instead of downloading the ``jupython.sif`` container file you've cloned the entire github repo
   (``git clone git@github.com:blzserdos/jupython.git``), have built and pushed the container, and have transfered it to your HPC environment.
   Then change your folder to the root of the ``jupython`` repository, and run these commands:

   ```
   singularity exec --bind reference/:/ref:ro,out_dir:/out:rw singularity/jupython.sif python ...
   ```
...
3. Run
   ...
   ```
   singularity shell --home $PWD:/home -B $(pwd)/data:/data jupython.sif
   ``` 
...
* Run singularity container within SLURM job scheduler, by creating a ``jupython_job.sh`` file (by adjusting the example below), and running ``sbatch jupython_job.sh``:

  ```
  #!/bin/bash
  #SBATCH --job-name=jupython
  #SBATCH --account=$PROJECT
  #SBATCH --time=00:10:00
  #SBATCH --cpus-per-task=1
  #SBATCH --mem-per-cpu=8000M
  module load singularity/3.7.1
  singularity exec --no-home jupython.sif python -c "print('Hello World')"
  singularity exec --home $PWD:/home jupython.sif python -c "print('Hello Moon')"
  ```

Please [let us know](https://github.com/blzserdos/jupython/issues/new) if you face any problems.
