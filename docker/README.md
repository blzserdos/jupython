...
If you face any issues, or if you need additional software, please let us know by creating an [issue](https://github.com/espenhgn/jupython/issues/new).
...
For convenience, a `Makefile` is provided in this directory in order to build [Singularity](https://docs.sylabs.io) containers from Dockerfiles (as `<jupython/docker/dockerfiles/jupython/Dockerfile>`).
...
If all went well, the built file should be located as `<jupython/containers/jupython.sif>`.
In case super-user (`sudo`) privileges are required, issue:

```
sudo make jupython.sif
```
...
```
docker build -t jupython -f dockerfiles/jupython/Dockerfile .  # build docker container
```

In case you do not want to use Singularity (e.g., for testing locally), the build can be used e.g., by issuing

```
docker run -it -p 5001:5001 jupython python --version
```
...
```
bash scripts/convert_docker_image_to_singularity.sh jupython  # produces jupython.sif
bash scripts/scripts/move_singularity_file.sh.sh jupython  # put jupython.sif file to <jupython>/containers/ directory
```
...
Some basic checks for the functionality of the different container builds are provided in `<jupython>/tests/`, implemented in Python.
...
Checks for individual containers (e.g., `jupython.sif`) can be executed by issuing:

```
py.test -v tests/test_jupython.py
```

Note that the proper container files (*.sif files) corresponding to the different test scripts must exist in `<jupython>/containers/`,
