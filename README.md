...
The remaining codes may then be added to and be modified further to suit the requirements of the `<jupython>` project. 
...
Revise the `<jupython>/.gitattributes` file as necessary. Some common file formats has been added already.
...
[![Flake8 lint](https://github.com/blzserdos/jupython/actions/workflows/python.yml/badge.svg)](https://github.com/blzserdos/jupython/actions/workflows/python.yml)
[![Dockerfile lint](https://github.com/blzserdos/jupython/actions/workflows/docker.yml/badge.svg)](https://github.com/blzserdos/jupython/actions/workflows/docker.yml)
[![Container build](https://github.com/blzserdos/jupython/actions/workflows/container_build.yml/badge.svg)](https://github.com/blzserdos/jupython/actions/workflows/container_build.yml)
[![Container build push](https://github.com/blzserdos/jupython/actions/workflows/container_build_push.yml/badge.svg)](https://github.com/blzserdos/jupython/actions/workflows/container_build_push.yml)

## Description of available containers

* ``jupython`` - a hello-world introductory container setup

## Software versions

Below is the list of tools included in the different Dockerfile(s) and installer bash scripts for each container.
Please keep up to date (and update the main `<jupython>/README.md` when pushing new container builds):
...
git clone --depth 1 https://github.com/blzserdos/jupython.git
cd jupython
git lfs pull  # pull "large" files
```

#### Update the `jupython.sif` container

To obtain updated versions of the Singularity Image Format (.sif) container file `, issue

```bash
cd path/to/repositories/jupython/singularity
mv jupython.sif jupython.sif.old  # optional, just rename the old(er) file
apptainer pull jupython.sif docker://ghcr.io/blzserdos/jupython:<tag>  # or
singularity pull jupython.sif docker://ghcr.io/blzserdos/jupython:<tag> # or 
oras pull ghcr.io/blzserdos/jupython_sif:<tag>
```

where `<tag>` corresponds to a tag listed under [packages](https://github.com/blzserdos/jupython/pkgs/container/jupython),
such as `latest`, `main`, or `sha_<GIT SHA>`. 
The `oras pull` statement pulls the `jupython.sif` file from [ghcr.io](https://github.com/blzserdos/jupython/pkgs/container/jupython_sif) using the [ORAS](https://oras.land) registry, without the need to build the container locally.
...
```bash
docker pull ghcr.io/blzserdos/jupython:<tag>
```
...
>
>```bash
>docker build --platform=linux/amd64 -t ghcr.io/blzserdos/jupython -f dockerfiles/jupython/Dockerfile .
>```
...
export IMAGE="ghcr.io/blzserdos/jupython:latest"  # adapt as necessary
...
Some secure platforms do not have direct internet access, hence we recommend cloning/pulling all required files on a machine with internet access as explained above, and archive the `jupython` directory with all files and moving it using whatever file uploader is available for the platform.

```bash
cd /path/to/jupython
SHA=$(git rev-parse --short HEAD)
cd ..
tar --exclude=".git/*" -cvf jupython_$SHA.tar jupython
```
...
For instructions on how to build or rebuild containers manually using [Docker](https://www.docker.com) and [Singularity](https://docs.sylabs.io) refer to [`<jupython>/docker/README.md`](https://github.com/blzserdos/jupython/blob/main/docker/README.md).

## Unit tests

Unit tests are available in the `<jupython>/tests` directory. 
...
`<jupython>/docs/source/_build/html/index.html`

The documentation may also be hosted online on [readthedocs.org](https://readthedocs.org).

## SLURM jobscript example

A basic job script example for running a Singularity container in an HPC setting with the [SLURM](https://slurm.schedmd.com) job scheduler is provided in the file [singularity_slurm_job.sh](https://github.com/blzserdos/jupython/blob/main/scripts/singularity_slurm_job.sh), and should be modified as needed.
It expects a few environment variables, and can be submitted as

```
export JOBNAME=jupython
...
```
The output of the job will be written to the text files `jupython.out` (output) and `jupython.err` (errors).


## Workflows

The following sections describe how to run the provided workflows using different workflow management systems.
The Docker container is assumed to be available and tagged as `ghcr.io/blzserdos/jupython:latest`: 

```bash
# (optional) make sure that the container is available; tagged as "latest"
docker pull --platform=linux/amd64 ghcr.io/blzserdos/jupython:<tag>
docker image tag ghcr.io/blzserdos/jupython:0.1.0rc8 ghcr.io/blzserdos/jupython:latest
```
...
We have included a basic WDL file and JSON file defining inputs in the `<jupython>/wdl` directory.
...
We have provided a basic Snakefile in the `<jupython>/snakemake` directory.
...
If you face any issues, or if you need additional software, please let us know by creating a new [issue](https://github.com/blzserdos/jupython/issues/new).
